...
const DELAY_ORDER_TIME = 10; //minutes
...
class BookingService {
	static createBooking = async ({
		...
		const { checkoutReview, checkoutOrder } =
			await CheckoutService.checkoutReview({ cartId, discount, tourIds });
		...
		try {
			for (let i = 0; i < checkoutReview.length; i++) {
				...
				if (lockResult) await releaseLock(lockResult.key);
			}
			...
			if (keyLocks.includes(null)) {
				...
				throw new BadRequestError(
					"Some of your added tours have been updated. Please try again",
				);
			}
	...
	static bookNow = async ({ user, tour }) => {
		let cartId;
		let itemExisting = (
			await checkTourExist(user, tour.tour, tour.startDate)
		)[0];
		if (!itemExisting) {
			...
			await cartModel.findOneAndUpdate(
				{
					_id: itemExisting._id,
					"tours.tour": tour.tour,
					"tours.startDate": tour.startDate,
				},
				{ $set: { "tours.$": tour } },
			);
	...
	static getListBookings = async (query) => {
		...
		const { total, docs: bookings } = await getAll(
			BookingModel,
			query,
			true,
			popOptions,
	...
	static getListBookingsByUser = async (userId, query) => {
		...
		const { total, docs: bookings } = await getMany(
			BookingModel,
			{
	...
	static getBookingDetails = async (bookingId) => {
		...
		if (!booking[0]) throw new BadRequestError("Not found booking");
		return booking[0];
	};
	...
	static deleteBooking = async (bookingId) => {
		...
		if (!result)
			throw new BadRequestError(
				"Not found booking or booking has been completed",
			);
		return null;
	};
}
